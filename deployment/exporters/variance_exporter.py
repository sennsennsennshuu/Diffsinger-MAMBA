...
class DiffSingerVarianceExporter(BaseExporter):
    ...
    def export_model(self, path: Path, model_name: str = None):
        ...
        # Load and save sub-models (simplify done in individual _torch_export steps)
        linguistic_onnx = onnx.load(self.linguistic_encoder_cache_path)
        ...
        if self.model.predict_dur:
            dur_predictor_onnx = onnx.load(self.dur_predictor_cache_path)
        ...
        if self.model.predict_pitch:
            # Merge pitch sub-models into single ONNX
    ...
    def _torch_export_model(self):
        ...
        if self.model.predict_pitch:
            ...
            condition = torch.rand((1, hparams['hidden_size'], 15), device=self.device, dtype=torch.float32)
            ...
            # Register as proper submodule for tracing (use '_backbone_module' to avoid property conflict)
            pitch_predictor.pitch_predictor.add_module('_backbone_module', traced_backbone)
            # Override the property to return the registered module
            pitch_predictor.pitch_predictor.__class__.backbone = property(lambda self: self._backbone_module)

            # PyTorch 2.9.0 dev bug workaround: export directly without jit
            print(f'Exporting {self.pitch_predictor_class_name} (direct export, no jit)...')
            class PitchPredictorExportWrapper(torch.nn.Module):
        ...
        if self.model.predict_variances:
            ...
            traced_var_backbone = torch.jit.trace(
            ...
            # Register as proper submodule for tracing (use '_backbone_module' to avoid property conflict)
            multi_var_predictor.variance_predictor.add_module('_backbone_module', traced_var_backbone)
            multi_var_predictor.variance_predictor.__class__.backbone = property(lambda self: self._backbone_module)

            # PyTorch 2.9.0 dev bug workaround: export directly without jit
            print(f'Exporting {self.multi_var_predictor_class_name} (direct export, no jit)...')
            class VarPredictorExportWrapper(torch.nn.Module):
                def __init__(self, predictor, steps):
                    super().__init__()
                    self.predictor = predictor
                    self.steps = steps
