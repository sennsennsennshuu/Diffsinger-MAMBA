...
class BaseTask(pl.LightningModule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # CPU optimization: set PyTorch threads for CPU training
        # 使用物理核心数，避免超线程竞争
        if not torch.cuda.is_available():
            cpu_count = os.cpu_count() or 4
            physical_cores = cpu_count // 2  # 假设超线程比例为 2
            optimal_threads = max(1, physical_cores)
            torch.set_num_threads(optimal_threads)
            torch.set_num_interop_threads(min(4, optimal_threads))
            # 设置环境变量，确保所有库使用相同的线程数
            os.environ["OMP_NUM_THREADS"] = str(optimal_threads)
            os.environ["MKL_NUM_THREADS"] = str(optimal_threads)
            os.environ["OPENBLAS_NUM_THREADS"] = str(optimal_threads)
            rank_zero_info(f'[CPU Training] CPU cores: {cpu_count}, physical: {physical_cores}')
            rank_zero_info(f'[CPU Training] PyTorch threads: {torch.get_num_threads()}, interop: {torch.get_num_interop_threads()}')
    ...
    def train_dataloader(self):
        ...
        # CPU optimization: disable pin_memory for CPU training
        is_cpu = not torch.cuda.is_available()
        pin_memory = False if is_cpu else True
        # Auto-adjust workers: GPU uses 2 (prefetch data while GPU computes, avoids 0→100% utilization swings)
        # CPU training uses ds_workers from config (auto-increased for better utilization)
        if is_cpu:
            num_workers = hparams['ds_workers']
            if num_workers < 4:
                num_workers = min(8, os.cpu_count() or 4)
                rank_zero_info(f'[CPU Training] Auto-increased ds_workers to {num_workers} for better CPU utilization')
        else:
            num_workers = 2
        return torch.utils.data.DataLoader(
            ...
            prefetch_factor=hparams['dataloader_prefetch_factor'] if num_workers > 0 else None,
    ...
    def val_dataloader(self):
        ...
        # CPU optimization: auto-adjust workers for CPU training
        is_cpu = not torch.cuda.is_available()
        if is_cpu:
            num_workers = hparams['ds_workers']
            if num_workers < 4:
            ...
            num_workers = 0  # GPU val: 0 workers to avoid spawn deadlock (audio logging needs main process)
        return torch.utils.data.DataLoader(
            self.valid_dataset,
