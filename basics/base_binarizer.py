...
from utils.multiprocess_utils import chunked_multiprocess_run, chunked_multiprocess_run_v2
...
class BaseBinarizer:
    ...
    def __init__(self, datasets=None, data_attrs=None):
        ...
        self.device = torch.device('cpu')  # 强制 CPU 预处理，避免 GPU 多进程共享问题
    ...
    def _init_worker(self):
        """Per-worker initialization, called once per process before processing items.
        Subclasses can override to pre-load heavy models (pitch extractor, etc)."""
        pass
    ...
    def check_coverage(self):
        ...
        if ph_idx_occurred != ph_idx_required:
            ...
            coverage_opt = self.binarization_args.get('coverage_check_option', 'strict')
            if coverage_opt == 'compat':
                print(f'| [compat] Warning: missing phonemes in transcriptions (ignored): {missing_phones}')
            else:
    ...
    def process_dataset(self, prefix, num_workers=0, apply_augmentation=False):
        ...
        try:
            if num_workers > 0:
                # CPU 并行化优化：设置环境变量，让每个 worker 知道总 worker 数
                # 这样每个 worker 可以动态分配线程数
                os.environ['BINARIZE_NUM_WORKERS'] = str(num_workers)
                # code for parallel processing (使用优化版：SimpleQueue + 无序收集 + 块分配)
                for item in tqdm(
                        chunked_multiprocess_run_v2(
                            self.process_item, args, num_workers=num_workers,
                            init_func=self._init_worker
                        ),
