...
class PhonemeDictionary:
                ...
                for line in dict_file:
                    ...
                    for phoneme in phonemes:
                        ...
                        if phoneme in all_phonemes:
                            continue
                        # Always add language prefix for consistent phoneme naming
    ...
    def encode_one(self, phone, lang=None):
        if '/' in phone:
            # Phone already has language prefix
            if phone in self._phone_to_id:
                return self._phone_to_id[phone]
            # Try to extract lang and lookup with short name
            lang, short_phone = phone.split('/', maxsplit=1)
            if short_phone in self._phone_to_id:
                return self._phone_to_id[short_phone]
        # Phone without prefix, add lang prefix if available
        if lang is not None:
            prefixed_phone = f'{lang}/{phone}'
            if prefixed_phone in self._phone_to_id:
                return self._phone_to_id[prefixed_phone]
