...
def set_hparams(config='', exp_name='', hparams_str='', print_hparams=True, global_hparams=True):
    ...
    args_work_dir = ''
    # 为 exp_name 提供默认值，避免文件散落项目根目录
    if args.exp_name == '' and args.config != '':
        # 从config文件名派生exp_name: data/config_acoustic.yaml → config_acoustic
        exp_from_config = os.path.splitext(os.path.basename(args.config))[0]
        args.exp_name = exp_from_config
    if args.exp_name != '':
        args.work_dir = args.exp_name
        args_work_dir = os.path.join('experiments', args.work_dir)
