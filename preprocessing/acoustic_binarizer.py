...
# CPU 并行化优化：根据 worker 数量动态分配线程
# 每个 worker 使用 1 个线程，通过多进程实现并行
# 这样可以避免 GIL 锁竞争，确保 CPU 均匀分配
_num_workers = int(os.environ.get('BINARIZE_NUM_WORKERS', '1'))
_cpu_count = os.cpu_count() or 1
_omp_threads = max(1, _cpu_count // max(1, _num_workers))
os.environ["OMP_NUM_THREADS"] = str(_omp_threads)
os.environ["MKL_NUM_THREADS"] = str(_omp_threads)
os.environ["OPENBLAS_NUM_THREADS"] = str(_omp_threads)
...
class AcousticBinarizer(BaseBinarizer):
    ...
    def _init_worker(self):
        """Per-worker 初始化：预加载音高提取器模型，避免每个 item 都加载一次"""
